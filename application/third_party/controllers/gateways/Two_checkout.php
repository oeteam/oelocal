...
class Two_checkout extends CI_Controller
{
    ...
    public function complete_purchase_booking()
    {
        if ($this->input->post()) {
            ...
            $data['amount']      =  xml_currency_change($total,agent_currency(),'USD');
            $data['currency']    = 'USD';
            $data['checkout']    = $this->Common_Model->checkoutdetails();
            $oResponse      = $this->two_checkout_gateway->finish_payment($data);
            ...
            if ($oResponse->isSuccessful()) {
                ...
                $oResponse = $oResponse->getData();
                if ($oResponse['response']['responseCode'] == 'APPROVED') {
                  $total = $oResponse['response']['total'];
                  $currency = $oResponse['response']['currencyCode'];
                  $transid = $oResponse['response']['merchantOrderId'];
                  redirect(site_url('payment/booking_online_payment_response?ordernumber='.$transactionid.'&amount='.$total.'&currency='.$currency.'&msg=success&transid='.$transid.'&gateway=2checkout'));
                }
            } elseif ($oResponse->isRedirect()) {
                $oResponse->redirect();
            } else {
    ...
    public function complete_purchase_xmlbooking()
    {
        if ($this->input->post()) {
            $data      = $this->input->post();
            $total = $this->input->post('total');
            ...
            if ($oResponse->isSuccessful()) {
                ...
                if ($oResponse['response']['responseCode'] == 'APPROVED') {
                  ...
                  $currency = $oResponse['response']['currencyCode'];
                  $transid = $oResponse['response']['merchantOrderId'];
                  redirect(site_url('payment/xml_booking_online_payment_response?ordernumber='.$transactionid.'&amount='.$total.'&currency='.$currency.'&msg=success&transid='.$transid.'&gateway=2checkout'));
                }
            } elseif ($oResponse->isRedirect()) {
                $oResponse->redirect();
            } else {
