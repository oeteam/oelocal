...
function HotelsView() {
  $ci =& get_instance();
  $result = $ci->db->query('select hotelid from home_page_banner order by rand() limit 1
')->result();
  if ($result[0]->hotelid!="") {
    $hotelId = explode(",", $result[0]->hotelid);
  ...
  if (count($query)!=0) {
        ...
        if (array_sum($BlackoutDateCheck)==0) {
          ...
          if ($query[0]->discountCode!="") {
            ...
            $BookingCode = $ci->db->query('select BookingCode from hotel_tbl_contract where contract_id = "'.$contract_id.'"')->result();
...
function backend_Aed_convertion($usr_c,$c_amount) {
  ...
  return number_format((float)$converted_amount, 3, '.', '');
}
...
function revenue_markup2($hotel_id,$contract_id,$agent_id,$date) {  
  $ci =& get_instance();
  $query = $ci->db->query("SELECT * FROM `hotel_tbl_revenue` where FIND_IN_SET(".$hotel_id.", IFNULL(hotels,'')) > 0 AND FIND_IN_SET('".$contract_id."', IFNULL(contracts,'')) > 0 AND FIND_IN_SET(".$agent_id.", IFNULL(Agents,'')) > 0 AND FromDate <= '".$date."' AND  ToDate >= '".$date."'")->result();
  if (count($query)!=0) {
    $return['Markup'] = $query[0]->Markup;
    $return['Markuptype'] = $query[0]->Markuptype;
    $return['ExtrabedMarkup'] = $query[0]->ExtrabedMarkup;
...
function amendmentemailNotification($booking_id) {
    $ci =& get_instance();
    $ci->load->library('email');
    $ci->load->model("Finance_Model");
    $mail_settings = mail_details();
    ...
    $ci->db->where('id',$booking[0]->agent_id);
    $agent=$ci->db->get()->result();
    //print_r($agent[0]->Preferred_Currency);

    $ci->db->select('*');
    $ci->db->from('hotel_tbl_hotels'); 
    $ci->db->where('hotel_tbl_hotels.id',$booking[0]->hotel_id);
    $hotel=$ci->db->get()->result();
    ...
    $amenddata = $ci->Booking_Model->getamendmentdata($booking_id);
    ...
    $remarks = $ci->Payment_Model->get_policy_contract($booking[0]->hotel_id,$booking[0]->contract_id);
    ...
    $subject = 'YOUR BOOKING RATE IS CHANGED (BOOKING ID : '.$booking[0]->booking_id.')';     
    $BookingMsg = 'YOUR BOOKING RATE IS CHANGED';
    $bookingTypeTag = 'Please confirm the below booking under ALLOTMENT/FREESALE';
         ...
         foreach ($cancellation as $Canckey => $Cancvalue) { 
          if ($Cancvalue->roomIndex==($key+1) || $Cancvalue->roomIndex=="") {
            ...
            $cancellationTerm.= '<tr>
              <td>'.date('d/m/Y',strtotime($booking[0]->Created_Date)).'</td>
              <td>'.date('d/m/Y',strtotime($booking[0]->check_in)).' </td>
              <td>'.$agent[0]->Preferred_Currency.' '.currency_type($agent[0]->Preferred_Currency,ceil($charge)).' ('.$Cancvalue->application.' )'.'</td>
            </tr>';
             } else { 
    ...
    /*Hotel mail start*/
      ...
      $subject1 = 'BOOKING RATE CHANGED FROM OTELSEASY.COM (BOOKING ID : '.$booking[0]->booking_id.')';
      $message1 = 'Dear Reservations,<br><br>
                  Warm greetings from Otelseasy!<br><br>
                  '.$bookingTypeTag.'<br><br>
...
function hotelnameGet($id) {
  $ci =& get_instance();
  $ci->db->select('CONCAT(a.hotel_name," ",ag.Agent_Code) as hotel_name');
  $ci->db->from('hotel_tbl_hotels a');
  $ci->db->join('hotel_tbl_agents ag',' a.supplierid = ag.id','left');
  $ci->db->where('a.id',$id);
  $query=$ci->db->get();
  $final =  $query->result();
