...
class CILogViewer {
    ...
    private static $levelsIcon = [
      ...
      'CUSTOM' => 'glyphicon glyphicon-exclamation-sign',
    ];
    ...
    const LOG_LINE_START_PATTERN = "/((INFO)|(ERROR)|(DEBUG)|(CUSTOM)|(ALL))[\s-\d:]+(-->)/";
    const LOG_DATE_PATTERN = "/(\d{4,}-[\d-:]{2,})\s([\d:]{2,})/";
    const LOG_LEVEL_PATTERN = "/^((ERROR)|(INFO)|(CUSTOM)|(DEBUG))/";
