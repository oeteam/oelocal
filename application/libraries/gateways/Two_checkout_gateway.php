...
class Two_checkout_gateway extends App_gateway
{
    ...
    public function process_payment($data,$checkout)
    {
        $this->ci->session->set_userdata(array(
            'total_2checkout' => preg_replace('[,]','',$data['amount'])
        ));
        redirect(site_url('gateways/two_checkout/make_payment'));
    }

    public function finish_payment($data)
    {
        $gateway = Omnipay::create('TwoCheckoutPlus_Token');
        $gateway->setAccountNumber($data['checkout'][0]->account_number);
        $gateway->setPrivateKey($data['checkout'][0]->private_key);
        $gateway->setTestMode($data['checkout'][0]->active);
