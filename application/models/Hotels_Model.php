  ...
  //   }
   ...
   public function hotel_rooms_list_count_check($id) {
    ...
    public function allotementBlkupdate($request) {
    	$currency =  hotel_currency_type($request['hotel_id']);
						       ...
						       $result[$i]= date('Y-m-d', strtotime($request['bulk-alt-fromDate']. ' + '.$i.'  days'));
					    		...
					    		if (count($query_out[$i])!=0) 
					    		{
					    			if ($contract_type[0]->contract_type!="Main") 
					    			{
					    				...
					    				{
								    		$data['amount'] = backend_Aed_convertion($currency,$request['bulk-alt-amount']);
								    		$data['allotement'] =  0;
								    		$data['cut_off'] =  0;
									    	$this->db->where('contract_id',$request['bulk_alt_contract_id']);
								    		$this->db->where('room_id',$value);
					    			...
					    			else 
					    			{
								    	...
								    	$dataLOG= array( 
												 ...
												 'allotement'		=> isset($data['allotement']) ? $data['allotement'] : '',
												 'cut_off' 			=> isset($data['cut_off']) ? $data['cut_off'] : '',
												 'contract_id'		=> $request['bulk_alt_contract_id'],
												 'CreatedDate'  	=> date('Y-m-d H:i:s'),
								     			 'CreatedBy'     	=> $this->session->userdata('id'),
								     			 'Status'			=> 'updated'
						    	...
						    	else 
						    	{
					    			if ($contract_type[0]->contract_type!="Main") 
					    			{
					    				$data1 = array('amount'			=> backend_Aed_convertion($currency,$request['bulk-alt-amount']),
							    					  'allotement'		=> 0,
							    					  'cut_off'			=> 0,
			...
			if(isset($request['update_terms']) && $request['update_terms']=="on") {
				if ($request['fromDate'] > $request['toDate']) {
					$msg = 'To date should greater than From date !';
						...
						$this->db->query("update hotel_tbl_boardsupplement set fromDate='".$request['fromDate']."',toDate='".$request['toDate']."' where season='".$request['season_id']."' and contract_id='".$request['contract_id']."' and hotel_id='".$request['hotel_id']."'");
						$this->db->query("update hotel_tbl_generalsupplement set fromDate='".$request['fromDate']."',toDate='".$request['toDate']."' where season='".$request['season_id']."' and contract_id='".$request['contract_id']."' and hotel_id='".$request['hotel_id']."'");
						$this->db->query("update hotel_tbl_cancellationfee set fromDate='".$request['fromDate']."',toDate='".$request['toDate']."' where season='".$request['season_id']."' and contract_id='".$request['contract_id']."' and hotel_id='".$request['hotel_id']."'");
			        		...
			        		if (in_array(date('D', strtotime($request['bulk-alt-fromDate']. ' + '.$i.'  days')), $_REQUEST['bulk-alt-days'])) {
						       ...
						       $result[$i]= date('Y-m-d', strtotime($request['bulk-alt-fromDate']. ' + '.$i.'  days'));
					    		...
					    		if (count($query_out[$i])!=0) 
					    		{
					    			if ($contract_type[0]->contract_type!="Main") 
					    			{
					    				if (isset($request['RwAmount'][$key])) {
								    		$data['amount'] = backend_Aed_convertion($currency,$request['RwAmount'][$key]);
								    		$data['allotement'] =  0;
								    		$data['cut_off'] =  0;
									    	$this->db->where('contract_id',$request['bulk_alt_contract_id']);
								    		$this->db->where('room_id',$value);
						    	...
						    	else 
						    	{
					    			if ($contract_type[0]->contract_type!="Main") 
					    			{
					    				$data1 = array('amount'			=> backend_Aed_convertion($currency,$request['RwAmount'][$key]),
							    					  'allotement'		=> 0,
							    					  'cut_off'			=> 0,
							    					  'allotement_date'	=> $result[$i],
							    					  'room_id'			=> $value,
