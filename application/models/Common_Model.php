...
class Common_Model extends CI_Model {
  ...
  public function HistoryLogs($module) {
    ...
    if ($module == 'Users') {
      ...
      $query = $this->db->query('SELECT *,id as userid,hotel_name as name,Updated_Date as dateVal,IF(Updated_By=" ","Hotel","Admin") as userType,"Update" as event,"Updated Existing  Hotel" as narration, IF(Updated_By!=" ",(SELECT CONCAT(First_Name, " ", Last_Name) FROM hotel_tbl_user WHERE id = a.updated_by),(SELECT CONCAT(First_Name, " ", Last_Name) FROM hotel_tbl_user WHERE id = a.id)) as userName FROM hotel_tbl_hotels as a where a.updated_date!=""')->result();
  ...
  public function BoardSupplementLogList($request) {
  ...
  public function GeneralSupplementLogList($request) {
    $this->db->select('a.*,d.hotel_name,CONCAT(e.First_Name,"",e.Last_Name) as Name');
  ...
  public function extrabedLogList($request) {
    $this->db->select('a.*,d.hotel_name,CONCAT(e.First_Name,"",e.Last_Name) as Name');
  ...
  public function allotmentLogList($request) {
    $this->db->select('a.*,d.hotel_name,CONCAT(e.First_Name,"",e.Last_Name) as Name');
  ...
  public function developerDetails($id){
      $this->db->select('*');
      $this->db->from('tbl_apiconnectdetails');
  ...
  public function developerUpdate($request) {
    $data= array(
              ...
              'IP_Test' =>implode(',',$_REQUEST['ip_test']),
              'IP_whitelist' =>implode(',',$_REQUEST['ip_whitelist']),
              'mode' => $request['mode'],
    ...
    $this->db->where('id',$request['edit_id']);
    $this->db->update('tbl_apiconnectdetails',$data);
    return true;
  }
  public function developerInsert($request) {
    ...
    $this->db->insert('tbl_apiconnectdetails',$data);
    return true;
  }
}
