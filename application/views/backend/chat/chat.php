...
//                            calculate_popups()
                ...
                if (res.error) {
                    ...
                    for (var i = 0; i < res.chats.length; i++) {
                        ...
                        if ($('#conversation_chat_' + chat.chatid).length) {
                            ...
                            $('#conversation_chat_' + chat.chatid+' ul').scrollTop($('#conversation_chat_' + chat.chatid+' ul')[0].scrollHeight);
                        }
                    }// end for loop
    ...
    function build_chat_area(data) {
        ...
        $('#open_chat_box_' + data.chatid).append('<div class="panel-heading custom-bg pt-sm"><div class="">' + data.title + '<div class="pull-right chat-icon">' +
            '<i data-toggle="tooltip" data-placement="top" onclick="minimize_chat_box(' + data.chatid + ')" title="minimize" class="fa fa-minus"></i><i data-toggle="tooltip" onclick="close_chat_box(' + data.chatid + ')" data-placement="top" title="<?= 'close' ?>" class="fa fa-times" aria-hidden="true"></i></div></div></div>');
                    ...
                    for (var i = 0; i < res.chats.length; i++) {
                        ...
                        if (!$('#open_chat_box_' + chat.chatid).length) {
                            ...
                            if ($('#open_chat_' + chat.chatid).length) {
                                ...
                                $('#open_chat_' + chat.chatid+' ul').scrollTop($('#open_chat_' + chat.chatid+' ul')[0].scrollHeight);
//                                alert('new');
                                // Now check
                                if (!nosound) {
        ...
        $.ajax({
            ...
            success: function (data) {
                if ($('#open_chat_' + id).length) {
                    ...
                    $('#open_chat_' + id+' ul').scrollTop($('#open_chat_' + id)[0].scrollHeight);
                    // Now check
                    if (nosound) {
