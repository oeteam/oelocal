<script type="text/javascript">
    ...
    (function poll(){
        $.ajax({
            url: base_url + "backend/chat/chatRequestCheck",
            type: "get",
            dataType: 'json',
            success: function (res) {
                salesGauge.setValue(res.value);
                if (res.count!=0) {
                    if ($("#chatRequestModal").attr('style')=="display: none;" || $("#chatRequestModal").attr('style')==undefined) {
                        $("#agent_id").val(res.agent_id);
                        $("#request_id").val(res.id);
                        $("#chatRequestModal").modal();
                        $("#chat-request").text(res.name+ " wants support from you");
        ...
        $('#chat_multi_user_button').click(function () {
            $.ajax({
                url: base_url + "backend/chat/load_multi_chat",
                type: "get",
                success: function (res) {
        ...
        $('.start_chat').click(function () {
            ...
            $.ajax({
                url: base_url + "backend/chat/open_chat_box?user_id="+user_id,
                type: "get",
                dataType: 'json',
                success: function (res) {
                    if (res.error) {
...
//                            calculate_popups()
    ...
    function open_chat_box(id, exist) {
        $.ajax({
            url: base_url + "backend/chat/active_chat_box/" + id,
            type: "get",
            dataType: 'json',
            success: function (res) {
                if (res.error) {
    ...
    function build_chat_area(data) {
        ...
        $('#open_chat_box_' + data.chatid).append('<div class="panel-heading custom-bg pt-sm"><div class="">' + data.title + '<div class="pull-right chat-icon">' +
            '<a class="dropdown-toggle" data-toggle="dropdown" aria-expanded="true"  title="setting" href="#"><i class="fa fa-cog" aria-hidden="true"></i><ul class="dropdown-menu chat-setting-dropdown animated zoomIn"><li><a href="' + base_url + "chat/conversations/" + data.chatid + '">Full Conversation"</a></li><li><a data-toggle="modal" data-target="#myModal"  href="' + base_url + "chat/change_title/" + data.chatid + '">Edit Name"</a></li> <li><a href="#" onclick="delete_chat_box(' + data.chatid + ')">Delete Conversation</a></li></ul></a><i data-toggle="tooltip" data-placement="top" onclick="minimize_chat_box(' + data.chatid + ')" title="minimize" class="fa fa-minus"></i><i data-toggle="tooltip" onclick="close_chat_box(' + data.chatid + ')" data-placement="top" title="<?= 'close' ?>" class="fa fa-times" aria-hidden="true"></i></div></div></div>');
    ...
    function all_chat_messages(nosound=0, myCallBack=null) {
        $.ajax({
            url: base_url + "backend/chat/all_chat_messages/",
            type: "get",
            dataType: 'json',
            success: function (res) {
                if (res.error) {
                    handle_error("#open_chat_box_" + 2, res.error_res);
                    return;
                } else {
...
//                                alert('new');
    ...
    function get_chat_messages(id, nosound=0) {
        ...
        $.ajax({
            url: base_url + "backend/chat/get_chat_messages/" + id,
            type: "get",
            dataType: 'JSON',
    ...
    function close_chat_box(id) {
        $.ajax({
            url: base_url + "backend/chat/close_chat_box/" + id,
            type: "get",
            dataType: 'json',
            success: function (res) {
                if (res.error) {
    ...
    function minimize_chat_box(id) {
        $.ajax({
            url: base_url + "backend/chat/minimize_chat_box/" + id,
            type: "get",
            dataType: 'json',
            success: function (res) {
                if (res.error) {
    ...
    function delete_chat_box(id) {
        $.ajax({
            url: base_url + "backend/chat/delete_chat_box/" + id,
            type: "get",
            dataType: 'json',
            success: function (res) {
                if (res.error) {
    ...
    function send_message(event, chatid) {
        ...
        if (e.keyCode == 13) {
            ...
            $.ajax({
                url: base_url + "backend/chat/send_message/" + chatid,
                type: "get",
                data: {
//                    hash: global_hash,
                    message: message
    ...
    function open_chats_boxes() {
        // alert(base_url + "backend/chat/open_chats_boxes");
        $.ajax({
            url: base_url + "backend/chat/open_chats_boxes",
            type: "get",
            data: {},
            dataType: 'JSON',
            success: function (res) {
    ...
    function accept_request() {
        ...
        $.ajax({
            url: base_url + "backend/chat/accept_request/"+agent_id+"/"+request_id,
            type: "get",
            dataType: 'json',
            success: function (res) {
                $(".start_chat").attr("data-user_id",agent_id).trigger('click');
                close_chatrequest_modal();
    ...
    function cancel_request(){
         var request_id =$("#request_id").val();
        $.ajax({
            url: base_url + "backend/chat/cancel_request/"+request_id,
            type: "get",
            dataType: 'json',
            success: function (res) {
                close_chatrequest_modal();
